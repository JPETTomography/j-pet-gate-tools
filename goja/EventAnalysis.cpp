...
void sort_hits(vector<Hit>& hits, string key) { sort(hits.begin(), hits.end(), EntityComp(key)); }
...
Hit merge_hits(const std::vector<Hit>& hits, const AveragingMethod winner = kCentroidWinnerEnergyWeightedFirstTime)
{
  ...
  case kCentroidWinnerEnergyWeightedFirstTime:
  {
    ...
    unsigned int min_index = std::distance(times.begin(), std::min_element(times.begin(), times.end()));
  ...
  case kEnergyWinner:
  {
    ...
    for (unsigned int i = 0; i < nHits; i++)
      energies.push_back(hits[i].edep);
    unsigned int max_index = std::distance(energies.begin(), std::max_element(energies.begin(), energies.end()));
...
std::tuple<int, int, std::vector<Hit>> select_coincident_hits(const vector<Hit>& hits, double compton_energy_threshold)
{
  ...
  {
    if (hits[i].edep > compton_energy_threshold)
      selected_hits.push_back(hits[i]);
  }
...
std::tuple<int, int, std::vector<Hit>> select_coincident_singles(const std::vector<Hit>& hits, double compton_energy_threshold,
                                                                 ...
                                                                 const AveragingMethod averagingMethod = kCentroidWinnerEnergyWeightedFirstTime)
{
...
EventType verify_type_of_coincidence_castor(const Hit& h1, const Hit& h2)
{
  ...
  {
    if (h1.nPhantomCompton == 0 && h2.nPhantomCompton == 0 && h1.nPhantomRayleigh == 0 && h2.nPhantomRayleigh == 0 && h1.nCrystalCompton == 1 &&
        h2.nCrystalCompton == 1 && h1.nCrystalRayleigh == 0 && h2.nCrystalRayleigh == 0)
      // true
      return kTrue;
    else
    {
      if (h1.nPhantomCompton == 0 && h2.nPhantomCompton == 0 && h1.nPhantomRayleigh == 0 && h2.nPhantomRayleigh == 0 &&
          (h1.nCrystalCompton > 1 || h2.nCrystalCompton > 1 || h1.nCrystalRayleigh > 0 || h2.nCrystalRayleigh > 0))
        // detector scat
        return kDetectorScattered;
      if (h1.nPhantomCompton == 1 || h2.nPhantomCompton == 1 || h1.nPhantomRayleigh == 1 || h2.nPhantomRayleigh == 1)
        // single scat
        return kPhantomScattered;
      if (h1.nPhantomCompton > 1 || h2.nPhantomCompton > 1 || h1.nPhantomRayleigh > 1 || h2.nPhantomRayleigh > 1)
        // multi scat
...
std::tuple<bool, std::vector<Hit>> check_gamma_type(const vector<Hit>& hits, double prompt_energy_threshold)
{
  ...
  bool isGood = prompt_multiplicity == 1;
...
void print_triple_coincidences(const std::vector<Hit>& hits, double PROMPT_E_TH)
{
  ...
  if (isGood)
  {
    ...
    cout << h3.edep << "\t";
    cout.precision(2);
    cout << h3.sourcePosX / 10. << "\t" << h3.sourcePosY / 10. << "\t" << h3.sourcePosZ / 10. << endl;
  }
}
...
void analyze_event(const vector<Hit>& hits, const bool hits_are_singles, const bool triple_coincidence)
{
  ...
  //if (N == MAX_N and N0 <= MAX_N0)
  // print_coincidences(selected_hits);
  ...
  else
  {
    ...
    if (N == MAX_N and N0 <= MAX_N0)
    {
      print_triple_coincidences(selected_hits, PROMPT_E_TH);
    };
  }
